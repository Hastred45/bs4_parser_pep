...
def latest_versions(session):
    '''Собирает информацию о версиях Python и ссылки на документацию'''
...
def pep(session):
    '''
    Получает данные обо всех документах PEP, сравнивает статус на
    ...
    for tr_tag in tqdm(tr_tags):
        ...
        if not response:
            continue
        soup = BeautifulSoup(response.text, 'lxml')
        dl_tag = find_tag(soup, 'dl', 'rfc2822 field-list simple')
...
def main():
    '''
    Основная логика. Логирует работу.
    Принимает команды из терминала. Очищает кэш.
    '''
    configure_logging()
